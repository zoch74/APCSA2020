...
public class ElevensBoard extends Board {
	...
	public boolean anotherPlayIsPossible() {
		...
		for (int k = 0; k < this.cardIndexes().size(); k++) {
			for (int j = k+1; j < this.cardIndexes().size(); j++) {
				temp.add(k);
				temp.add(j);
				...
				for (int l = j+1; l < this.cardIndexes().size(); l++) {
					temp.add(l);
					System.out.println(temp);
					if (isLegal(temp)) {
						return true;
	...
	private boolean containsJQK(List<Integer> selectedCards) {
		...
		if (selectedCards.size() == 3) {
			for (int k = 0; k < selectedCards.size(); k++) {
				if (this.cardAt(selectedCards.get(k)).rank().equals("jack")){
					jack = true;
				}
				if (this.cardAt(selectedCards.get(k)).rank().equals("king")){
					king = true;
				}
				if (this.cardAt(selectedCards.get(k)).rank().equals("queen")){
					queen = true;
				}
			}
